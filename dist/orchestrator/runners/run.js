...
exports.TaskType = exports.CommandType = void 0;
exports.run = run;
...
const CommandSchema = zod_1.z.object({
    ...
    commandType: zod_1.z.nativeEnum(CommandType),
...
const TaskSchema = zod_1.z.object({
    ...
    agent: zod_1.z.nativeEnum(TaskType),
    action: zod_1.z.string(),
    description: zod_1.z.string(),
    payload: zod_1.z.record(zod_1.z.any()),
    dependencies: zod_1.z.array(zod_1.z.string()).default([]),
    customDependencies: zod_1.z.array(zod_1.z.string()).optional(),
    deploymentGroup: zod_1.z.string().optional(),
    ...
    resourceRequirements: zod_1.z.object({
        cpu: zod_1.z.number().min(1).max(8).default(1),
        memory: zod_1.z.number().min(1).max(16).default(2)
    }).default({ cpu: 1, memory: 2 })
}).transform(data => ({
    ...data,
    status: data.status || 'pending'
}));
const ExecutionStageSchema = zod_1.z.object({
    stageId: zod_1.z.string(),
    parallelTasks: zod_1.z.array(zod_1.z.string()),
    dependencies: zod_1.z.array(zod_1.z.string()).default([]),
    estimatedDuration: zod_1.z.number().positive(),
    resourceRequirements: zod_1.z.object({
        maxCpu: zod_1.z.number().positive(),
        maxMemory: zod_1.z.number().positive(),
        maxConcurrency: zod_1.z.number().positive()
    })
});
const QualityGateSchema = zod_1.z.object({
    gate: zod_1.z.string(),
    required: zod_1.z.boolean(),
    trigger: zod_1.z.enum(['before_start', 'before_deploy', 'after_completion']),
    checks: zod_1.z.array(zod_1.z.string()).optional(),
    thresholds: zod_1.z.record(zod_1.z.number()).optional()
});
const ExecutionPlanSchema = zod_1.z.object({
    planId: zod_1.z.string().uuid(),
    commandId: zod_1.z.string().uuid(),
    tasks: zod_1.z.array(TaskSchema),
    executionStages: zod_1.z.array(ExecutionStageSchema),
    qualityGates: zod_1.z.array(QualityGateSchema),
    estimatedTotalDuration: zod_1.z.number().positive(),
    totalResourceRequirements: zod_1.z.object({
        maxParallelTasks: zod_1.z.number().positive(),
        totalCpuUnits: zod_1.z.number().positive(),
        totalMemoryUnits: zod_1.z.number().positive()
    }),
    metadata: zod_1.z.object({
        createdAt: zod_1.z.string().datetime(),
        complexity: zod_1.z.enum(['simple', 'medium', 'complex']).optional(),
        requiredAgents: zod_1.z.array(zod_1.z.nativeEnum(TaskType)).optional(),
        aiGenerated: zod_1.z.boolean().optional(),
        model: zod_1.z.string().optional()
    })
});
const OrchestratorResultSchema = zod_1.z.object({
    success: zod_1.z.boolean(),
    plan: ExecutionPlanSchema.optional(),
    error: zod_1.z.string().optional(),
    warnings: zod_1.z.array(zod_1.z.string()).default([]),
    chatResponse: zod_1.z.object({
    ...
    metadata: zod_1.z.object({
        processingTimeMs: zod_1.z.number().positive(),
...
function createExecutionStages(tasks) {
    ...
    return [{
            stageId: crypto_1.default.randomUUID(),
...
async function initializeLLM() {
    if (!llmAdapter) {
        llmAdapter = new llmAdapter_1.LLMAdapter();
    ...
    try {
        ...
        if (aiResult.aiResponse.success && aiResult.aiResponse.plan) {
            ...
            return {
                success: true,
                plan: {
                    planId: aiPlan.planId || crypto_1.default.randomUUID(),
                    commandId: command.commandId,
                    tasks: aiPlan.tasks || [],
                    executionStages: [],
