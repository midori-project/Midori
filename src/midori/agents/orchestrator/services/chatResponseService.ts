...
export interface ChatResponse {
  message: string;
  tone: 'friendly' | 'professional' | 'helpful' | 'encouraging';
  suggestions?: string[];
...
export interface UnifiedResponse {
  ...
  analysis: {
    ...
    conversationStage: 'greeting' | 'planning' | 'execution' | 'completion';
  };
}
...
export class ChatResponseService {
  ...
  // 🎯 วิเคราะห์ประเภทการตอบสนอง
  private analyzeResponseType(userInput: string, intent: string): string {
  ...
  // 💬 สร้างข้อความตอบกลับ
  private generateMessage(responseType: string, intent: string, context: Record<string, any>): string {
  ...
  private selectTone(responseType: string): ChatResponse['tone'] {
    const toneMap: Record<string, ChatResponse['tone']> = {
      greeting: 'friendly',
  ...
  // 💡 สร้างคำแนะนำ
  private generateSuggestions(responseType: string, context: Record<string, any>): string[] {
  ...
  // ❓ สร้างคำถามติดตาม
  private generateFollowUpQuestions(responseType: string, context: Record<string, any>): string[] {
    if (responseType === 'creation_request') {
      return [
  ...
  createUnifiedResponse(
    ...
    const chatResponse = this.createChatResponse(
      userInput,
      analysisResult.analysis?.userIntent || 'general',
      {
  ...
  private determineConversationStage(userInput: string, analysisResult: any): UnifiedResponse['analysis']['conversationStage'] {
    const lower = userInput.toLowerCase();
