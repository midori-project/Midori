...
} from "../template-system/business-categories";
...
export class CategoryService {
  ...
  private initialize() {
    try {
      if (process.env.OPENAI_API_KEY) {
        ...
        console.warn(
          "⚠️ No OpenAI API key found, Category Service will use keyword matching only"
        );
  ...
  async detectCategory(
    options: CategoryDetectionOptions
  ): Promise<BusinessCategoryManifest | undefined> {
    const {
      id,
      keywords,
      userInput,
    ...
    if (fallbackToDefault) {
      ...
      return (
        BUSINESS_CATEGORIES.find((c) => c.id === "ecommerce") ||
   ...
   */
  private async detectCategoryByKeywords(
    keywords: string[]
  ): Promise<BusinessCategoryManifest | undefined> {
      ...
      for (const keyword of keywords) {
        ...
        if (
          category.keywords.some(
            (catKeyword) => catKeyword.toLowerCase() === keywordLower
          )
        ) {
          score += 5;
          continue;
        ...
        const foodKeywords = ["อาหาร", "ร้านอาหาร", "restaurant", "food"];
        ...
        const bookKeywords = ["ขายของ", "ร้านขายของ", "store"];
        ...
        if (
          portfolioKeywords.some((portfolio) =>
            keywordLower.includes(portfolio)
          )
        ) {
    ...
    if (bestMatch && bestMatch.score >= 2) {
      console.log(
        `Selected category: ${bestMatch.category.id} with score: ${bestMatch.score}`
      );
   ...
   */
  private async detectCategoryByLLM(
    userInput: string
  ): Promise<BusinessCategoryManifest | undefined> {
    try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a business category classifier. Respond with only the category ID.",
          },
          {
            role: "user",
            content: prompt,
