...
export class OverrideSystem {
  ...
  async resolveManifest(
    ...
    try {
      ...
      const result = this.resolver.resolveManifest(businessCategoryId, customOverrides);
      ...
      this.errorHandler.handleError(error as Error, { businessCategoryId, customOverrides });
      throw error;
    }
  }
  ...
  async renderTemplates(
    ...
    try {
      ...
      this.errorHandler.handleError(error as Error, { concreteManifest, userData });
      throw error;
    }
  }
  ...
  async generateWebsite(
    ...
    try {
      this.logger.logBusinessCategorySelection(businessCategoryId, userData.keywords || []);

      // Step 1: Resolve Manifest
      const resolverResult = await this.resolveManifest(businessCategoryId, customOverrides);
      ...
      this.errorHandler.handleError(error as Error, { businessCategoryId, userData });
      throw error;
    }
  }
...
export function createAIPrompt(config: AIPromptConfig): string {
  const { businessCategory, concreteManifest, keywords, customInstructions } = config;
...
Generate content that matches the schema exactly. Return JSON with the following structure:
{
  ...
  "${blockKey}": {`;
    ...
    for (const [placeholder, config] of Object.entries(block.placeholders)) {
      const description = config.description || `appropriate ${config.type} value`;
...
function getArrayExample(placeholder: string, blockId: string): string {
  const examples: Record<string, Record<string, string>> = {
    'navbar-basic': {
      'menuItems': '{ "label": "หน้าแรก", "href": "/" }, { "label": "เกี่ยวกับเรา", "href": "/about" }, { "label": "ติดต่อ", "href": "/contact" }',
      'socialLinks': '{ "name": "Facebook", "url": "https://facebook.com", "icon": "📘" }, { "name": "Instagram", "url": "https://instagram.com", "icon": "📷" }'
    },
    'about-basic': {
      'features': '{ "title": "คุณสมบัติ 1", "description": "คำอธิบาย" }, { "title": "คุณสมบัติ 2", "description": "คำอธิบาย" }, { "title": "คุณสมบัติ 3", "description": "คำอธิบาย" }',
      'stats': '{ "number": "100+", "label": "ลูกค้า" }, { "number": "5★", "label": "รีวิว" }, { "number": "24/7", "label": "บริการ" }, { "number": "100%", "label": "ความพึงพอใจ" }'
    },
    'footer-basic': {
      'socialLinks': '{ "name": "Facebook", "url": "https://facebook.com", "icon": "📘" }, { "name": "Instagram", "url": "https://instagram.com", "icon": "📷" }, { "name": "Line", "url": "https://line.me", "icon": "💬" }',
      'quickLinks': '{ "label": "หน้าแรก", "href": "/" }, { "label": "เกี่ยวกับเรา", "href": "/about" }, { "label": "ติดต่อ", "href": "/contact" }'
    }
  };
