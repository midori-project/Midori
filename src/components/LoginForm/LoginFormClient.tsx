  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.remember) {
        formDataToSend.append('remember', 'on');
      }

      const result = await loginAction({ ok: true }, formDataToSend);

      if (result.ok) {
        // อัปเดต AuthContext state หลังจาก login สำเร็จ
        await refetchUser();
        // Notify other tabs that login happened
        try {
          localStorage.setItem('midori-auth-change', JSON.stringify({ type: 'login', t: Date.now() }));
        } catch (e) {
          /* ignore */
        }
    ...
    <div className="w-full mx-auto">
      <div className="shadow-xl rounded-xl border border-gray-100 p-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Email Field (single, as in Figma) */}
          <div>
            <label
              htmlFor="email"
        ...
        <div className="mt-4 text-center">
          <div className="text-sm text-gray-600">
            ...
            <a
              href="/signup"
              className="text-emerald-600 underline hover:text-emerald-500 font-medium transition-colors"
            >
              Create your account
            </a>
          </div>
